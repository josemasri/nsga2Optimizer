...
NSGA::NSGA(ProblemFunction *mop, Randomizer *r, string outputDir, NSGAParams &gaParams)
{
   this->mop = mop;
   this->r = r;
...
//   initialize_pop(mixed_pop);
   ...
   report_pop_2files(parent_pop, 1);
...
void NSGA::evaluate_pop(Population *pop) {
      mop->evaluatePop(pop->ind, popsize);
}

void NSGA::evaluateIndividual(int i) {
    // r->randomize();
    // createInitialPopulation();
    // parent_pop->evaluate_ind(mop, i);
}
